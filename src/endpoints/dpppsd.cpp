...
struct dpppsd::endpoint_impl {

	endpoint_impl(double sampling_period_ns)
		: _logger{library_logger::create_logger("dpppsd_ep"s)}
		, _buffer()
		, _args_list{dpppsd::default_data_format()}
...
void dpppsd::decode_hit(const caen::byte*& p) {
	...
	if (is_last_word) {
		...
		caen::bit::mask_and_right_shift<hit_evt::s::energy_short>(word, evt._energy_short);
		...
		while (!is_last_word) {
			...
			case hit_evt::extra_type::wave_info: {
				...
				for (auto& probe : ed._analog_probes) {
					...
					probe._decoded_type = [](auto t) noexcept {
						...
						case s_ed::analog_probe::type::baseline:						return dpp_analog_probe_type::baseline;
						case s_ed::analog_probe::type::cfd:								return dpp_analog_probe_type::cfd;
				...
				for (auto& probe : ed._digital_probes) {
					...
					probe._decoded_type = [](auto t) noexcept {
						...
						case s_ed::digital_probe::type::over_threshold:					return dpp_digital_probe_type::over_threshold;
						case s_ed::digital_probe::type::charge_ready:					return dpp_digital_probe_type::charge_ready;
						case s_ed::digital_probe::type::long_gate:						return dpp_digital_probe_type::long_gate;
						case s_ed::digital_probe::type::event_pile_up:					return dpp_digital_probe_type::event_pile_up;
						case s_ed::digital_probe::type::short_gate:						return dpp_digital_probe_type::short_gate;
			...
			case hit_evt::extra_type::time_info: {
				...
				BOOST_ASSERT_MSG(!stats_time_info, "info data should not be already initialized");
			...
			case hit_evt::extra_type::counter_info: {
				...
				BOOST_ASSERT_MSG(special_event, "special_event flag should be set");
				BOOST_ASSERT_MSG(!has_waveform, "has_waveform flag should not be set");
				BOOST_ASSERT_MSG(!stats_counter_info, "info data should not be already initialized");
...
dpppsd::args_list_t dpppsd::default_data_format() {
	using vt = data_format_utils<dpppsd>::args_type;
	return {{
...
std::size_t dpppsd::data_format_dimension(names name) {
	switch (name) {
	case names::CHANNEL:
...
void dpppsd::set_data_format(const std::string& json_format) {
	_pimpl->set_data_format(json_format);
}

void dpppsd::read_data(timeout_t timeout, std::va_list* args) {
	...
	for (const auto& arg : _pimpl->_args_list) {
		...
		case names::ENERGY:
			...
			break;
		case names::ENERGY_SHORT:
			utility::put_argument(args, type, evt._energy_short);
			break;
		case names::FLAGS_LOW_PRIORITY:
...
void dpppsd::has_data(timeout_t timeout) {
...
struct dpppsd::stats::endpoint_impl {
	...
	void set_data_format(const std::string& json_format) {
		data_format_utils<dpppsd::stats>::parse_data_format(_args_list, json_format);
	}
...
dpppsd::stats::stats(client& client, handle::internal_handle_t endpoint_handle)
...
dpppsd::stats::~stats() = default;

dpppsd::stats::args_list_t dpppsd::stats::default_data_format() {
	using vt = data_format_utils<dpppsd::stats>::args_type;
	return {{
...
std::size_t dpppsd::stats::data_format_dimension(names name) {
	switch (name) {
...
void dpppsd::stats::set_data_format(const std::string& json_format) {
	_pimpl->set_data_format(json_format);
}

void dpppsd::stats::read_data(timeout_t timeout, std::va_list* args) {
...
void dpppsd::stats::has_data(timeout_t timeout) {
	boost::ignore_unused(timeout);
}
